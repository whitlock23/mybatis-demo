...
- 官网查询文档 https://mybatis.org/mybatis-3/zh/getting-started.html
...
- 在resource中配置文件mybatis-config.xml
  ...
  - 以mysql 为例
    ...
    - ```xml
      ...
      <configuration>
          ...
          <mappers>
      <!--        sql 映射文件-->
      <!--        <mapper resource="com/learning/mapper/UserMapper.xml"/>-->
              <package name="com.learning.mapper"/>
          </mappers>
  ...
  - resource 中配置logbook.xml
    ...
    - ```xml
      ...
      <configuration>
          ...
          <logger name="com.learning" level="DEBUG" additivity="false">
...
* #### 实体类 Brand
  ...
  * > orm 思想，一条数据为一个类的对象
  ...
  在 `com.learning.pojo` 包下创建 Brand 实体类。
...
在 `com.learning.mapper` 包写创建名为 `BrandMapper` 的接口。并在该接口中定义 `List<Brand> selectAll()` 方法。
...
在 `reources` 下创建 `com/learning/mapper` 目录结构，并在该目录下创建名为 `BrandMapper.xml` 的映射配置文件
...
> `resultType` 	可以直接写brand 是因为`	mybatis-config.xml` 中起了别名
>
> ```xml
>     <mappers>
> <!--        sql 映射文件-->
...
在 `test/java` 下的 `com.learning.mapper`  包下的 `MybatisTest类中` 定义测试方法
...
> 以后主要处理的事根据业务逻辑处理参数变量的过程
>
> 重点在于查询与sql 语句吻合的参数调整
